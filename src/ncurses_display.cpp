...
void NCursesDisplay::DisplaySystem(System& system, WINDOW* window) {
    ...
    mvwprintw(window, ++row, 2, "System Up Time : %s", Format::ElapsedTime(system.UpTime()).c_str());
...
void NCursesDisplay::DisplaySystemInfo(System& system, WINDOW* window) {
    ...
    mvwprintw(window, ++row, 2, "Operating System: %s", system.OperatingSystem().c_str());
...
void NCursesDisplay::DisplayProcesses(vector<Process>& processes, WINDOW* window, int n) {
    int row{0};
    ...
    int process_count = min((int)processes.size(), n); // Limit the number of processes to avoid memory issues
    ...
    for (int i = 0; i < process_count; ++i) {
        ...
        mvwprintw(window, row, command_column, "%s", processes[i].Command().substr(0, min((int)processes[i].Command().size(), window->_maxx - 85)).c_str());
    }
}
